...
def BC(inputImg):
    #img = cv.imread(r"D:\testFiles\BrightCircle\IMG_0012.jpg")
    img = inputImg
    gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
    # 挖去中心区域
    kernel = np.ones((70, 70), dtype=np.uint8)
    dilate = cv.dilate(gray, kernel, 4)
    #cv.imshow("dilate", dilate)
    #cv.waitKey(3000)
    ret, mask_ = cv.threshold(dilate, 120, 255, cv.THRESH_BINARY_INV)#阈值1
    cutImg = cv.bitwise_and(gray, gray, mask=mask_)
    ...
    ret, mask1 = cv.threshold(cutImg, 10, 255, cv.THRESH_BINARY)#阈值2
    cv.namedWindow(dir,cv.WINDOW_NORMAL)
    cv.imshow(dir, mask1)
    cv.imwrite(path+"\\OutputImg\\"+dir,mask1)
    cv.waitKey(2000)
